...
- Explain `replaceExplorer: true` default, its zero-config replacement purpose, the narrowly scoped
  CSS/`:has()` dependency on current Quartz default-frame/Explorer markup, and the `false` opt-out.
...
- Explain that sidebar book ordering/eligibility inherits normalized `excludeDirs`,
  ...
  containment at tablet/mobile breakpoints, mobile left-container width/wrap containment, and direct
  Explorer sibling suppression. State that Explorer is the only suppression and no rule targets the
  right rail, Graph, TOC, CanvasPage, or another custom frame.
