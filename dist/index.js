...
var sidebar_inline_default = `var d;function l(){try{return window.matchMedia?.("(max-width: 800px)").matches??window.innerWidth<=800}catch{return window.innerWidth<=800}}function u(r){let i=Array.from(document.querySelectorAll(".rip-sidebar .rip-sidebar-switcher"));for(let e of i){let o=()=>{if(e.open)for(let a of i)a!==e&&(a.open=!1)},n=a=>{a.target?.closest?.("a")&&(e.open=!1)};e.addEventListener("toggle",o),e.addEventListener("click",n),r.push(()=>{e.removeEventListener("toggle",o),e.removeEventListener("click",n)})}if(i.length===0)return;let t=e=>{let o=e.target;if(o)for(let n of i)n.open&&!n.contains(o)&&(n.open=!1)},s=e=>{if(e.key!=="Escape")return;let o=i.find(a=>a.open);if(!o)return;e.preventDefault(),o.open=!1,o.firstElementChild?.focus?.()};document.addEventListener("pointerdown",t),document.addEventListener("keydown",s),r.push(()=>{document.removeEventListener("pointerdown",t),document.removeEventListener("keydown",s)})}function f(r){let i=Array.from(document.querySelectorAll(".rip-sidebar [data-rip-disclosure]"));for(let t of i){let s=()=>{let e=t.getAttribute("aria-controls");if(!e)return;let o=document.getElementById(e),n=t.closest(".rip-sidebar-folder");if(!o||!n)return;let c=!(t.getAttribute("aria-expanded")==="true");t.setAttribute("aria-expanded",String(c)),o.hidden=!c,n.dataset.ripOpen=String(c)};t.addEventListener("click",s),r.push(()=>t.removeEventListener("click",s))}}function p(r){let i=Array.from(document.querySelectorAll(".rip-sidebar .rip-sidebar-explorer"));if(d!==void 0){let t=d;d=void 0,window.location.pathname===t&&!document.querySelector('.page[data-frame="canvas"]')&&i.forEach(s=>{s.open=!1})}for(let t of i){let s=e=>{if(e.defaultPrevented||!l()||e.button!==0||e.altKey||e.ctrlKey||e.metaKey||e.shiftKey)return;let n=e.target?.closest?.("a");!n||!t.contains(n)||n.getAttribute("aria-current")==="page"||n.dataset.ripNodeKind==="canvas"||(d=new URL(n.href,window.location.href).pathname,t.open=!1)};t.addEventListener("click",s),r.push(()=>t.removeEventListener("click",s))}}function m(){let r=[];u(r),p(r),f(r),r.length>0&&typeof window<"u"&&window.addCleanup&&window.addCleanup(()=>r.forEach(i=>i()))}typeof document<"u"&&document.addEventListener("nav",()=>m());
`;
